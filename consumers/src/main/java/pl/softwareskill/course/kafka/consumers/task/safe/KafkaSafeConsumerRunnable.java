...
class KafkaSafeConsumerRunnable implements Runnable {
    ...
    private void processEvents(RebalanceListner rebalanceListner) {
        while (!closed.get()) {
            ConsumerRecords<String, Text> records = consumer.poll(Duration.ofMillis(100));
            for (ConsumerRecord<String, Text> record : records) {
                processSingleEvent(rebalanceListner, record);
            }
        }
    }

    private void processSingleEvent(RebalanceListner rebalanceListner, ConsumerRecord<String, Text> record) {
